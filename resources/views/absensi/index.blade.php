...
<thead>
<tr>
<th><span class="text-dark-75">Nama / Tujuan Absensi</span></th>
<th><span class="text-dark-75">Link Download Absensi</span></th>
@if(Auth::user()->role == 1)
...
<tr>
<td class="pl-3 py-3">{{$absensi->absensi_nama}}</td>
<td class="pl-3 py-3"><a href="{{url($absensi->absensi_link)}}">{{$absensi->absensi_link}}</a></td>

@if(Auth::user()->role == 1)
<td>
    <a href="{{url('/absensi/edit/'.$absensi->absensi_id)}}" class="btn btn-xs btn-icon btn-warning"><i class="fas fa-edit nopadding"></i></a>
    <button type="button" href="{{url('/absensi/delete/'.$absensi->absensi_id)}}" class="deletebtn btn btn-xs btn-icon btn-danger"><i class="fas fa-trash nopadding"></i></button></td>
</td>
@endif
</tr>
@endforeach
