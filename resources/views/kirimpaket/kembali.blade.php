@extends('layouts.app')
@section('title','Pengembalian Produk Harian - ')
@section('css')
...
<span class="card-label font-weight-bolder text-dark">Pengembalian Produk Harian</span>
</h3>
...
<div class="tab-content">
    <form method="POST" action="{{url('turunbarang/kembalikan')}}">
        @csrf

        <input type="hidden" name="barangturun_id" value="{{$kembali->barangturun_id}}">
        <div class="form-group row mt-4">
            <label class="col-md-2">SKU Barang</label>
            <div class="col-md-3">
                 {{$kembali->barangturun_sku}}
            </div>
          </div>
          <div class="form-group row mt-4">
            <label class="col-md-2">Nama Petugas</label>
            <div class="col-md-3">
             {{$kembali->barangturun_namapetugas}}
            </div>
          </div>
          <div class="form-group row mt-4">
            <label class="col-md-2">Tanggal Ambil</label>
            <div class="col-md-3">
             {{$kembali->barangturun_tanggalambil}}
            </div>
          </div>
          <div class="form-group row mt-4">
            <label class="col-md-2">Tanggal Kembali</label>
            <div class="col-md-3">
            <input type="text" class="form-control" id="datepicker1" name="barangturun_tanggalkembali">
            </div>
          </div>
