		...
		<table class="table table-bordered mt-5" id="product">
			...
			<tbody>
				...
				<tr>
					...
					<td id="stok{{$p->product_sku}}">{{$p->publish_stok}}
                        <input type="hidden" id="stokakhir" name="stokakhir[]" value="{{$p->publish_stokakhir}}">
                        <input type="hidden"  id="stok" name="stok[]" value="{{$p->publish_stok}}"></td>
                        <td>{{$p->stokterjual}}
                ...
                success: function (data) {
                    if (data['success']) {
                        Swal.fire(
                         ...
                         window.location.href = "{{url('/stokopname/mingguan/edit/'.$pub)}}";
