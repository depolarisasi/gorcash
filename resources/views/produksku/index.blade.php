@extends('layouts.app')
@section('title','Database Artikel per SKU - ')
@section('css')
...
<span class="card-label font-weight-bolder text-dark">Database Artikel Per SKU</span>
</h3>
...
<!--end::Search Form-->
		...
		<table class="table table-bordered mt-5" id="product">
			...
			<tbody>
                ...
                @foreach($produk as $p)
				<tr data-row-id="{{$p->product_sku}}" @if($p->product_stok < 1 || $p->product_status == 1) class="ignore fsm psm" @else class="fsm psm" @endif>
                    <td></td>
					<td>{{$p->product_sku}}</td>
