            ...
            <div class="col-md-6">
                <div class="form-group row mt-4">
                    ...
                    <div class="form-group row mt-4">
                        ...
                        <div class="col-md-8">
                            <input class="form-control" id="tanggalpenjualan" type="datetime-local" name="penjualan_tanggalwaktupenjualan" value="{{\Carbon\Carbon::now()->setTimezone('Asia/Jakarta')->format('Y-m-d H:i:s')}}" required/>
                        </div>
                      </div>
            </div>
        </div>
...
total = total+parseInt(hargabaru)
document.getElementById('total').innerHTML = formatter.format(total);
}
...
$('#IQ'+produk).val(qtyordered);
    ...
    total = parseInt(hargabaru)
    document.getElementById('total').innerHTML = formatter.format(total);
    tambahharga(hargabaru);
}
            },
            error: function(data) {
        console.log('Cannot retrieve data.');
...
produk = this.getAttribute('data-idproduct');
qtyordered = parseInt(document.getElementById('qty'+produk).getAttribute('data-qtyordered'));
priceordered = parseInt(document.getElementById('price'+produk).getAttribute('data-priceordered'));
        $.ajax({
          url: '/productapi/getproduct',
          type: 'POST',
          data: {
...
$('#IQ'+produk).val(qtyordered);
    ...
    console.log(document.getElementById('price'+produk).innerHTML);
}
            },
            error: function(data) {
        console.log('Cannot retrieve data.');
